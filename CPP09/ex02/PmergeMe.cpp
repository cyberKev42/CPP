...
/*   Updated: 2025/06/26 19:02:08 by kbrauer          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
 ...
 * so my customized j_number starts from 1.
 * _jacobsthal_number(1) = 1
 * _jacobsthal_number(2) = 3
 * _jacobsthal_number(3) = 5
 * ...
 */
...
template <typename T> void PmergeMe::_sort_and_recurse(T& container, int pair_size) {
    ...
    Iterator last = move_iter(container.begin(), pair_size * nbr_of_pairs);
    Iterator end = move_iter(last, -(is_odd * pair_size));

    int jump = 2 * pair_size;
    for (Iterator it = container.begin(); it != end; it += jump) {
        Iterator this_pair = move_iter(it, pair_size - 1);
        Iterator next_pair = move_iter(it, (pair_size * 2) - 1);
        if (compare(next_pair, this_pair))
            _swap_pair(this_pair, pair_size);
    }
    _merge_insertion_sort(container, pair_size * 2);
}
...
template <typename T> void PmergeMe::_fill_chains(T& container, int pair_size, std::vector<typename T::iterator>& main, std::vector<typename T::iterator>& pend, bool is_odd, typename T::iterator end) {
    ...
    /* Fill main chain with b1 and a1 */
...
template <typename T> void PmergeMe::_insert_pend_into_main(std::vector<typename T::iterator>& main, std::vector<typename T::iterator>& pend, bool is_odd) {
    ...
    for (int k = 2;; k++) {
        ...
        while (inserts_to_do) {
            typename std::vector<typename T::iterator>::iterator insert_pos = std::upper_bound(main.begin(), bound_it, *pend_it, compare<typename T::iterator>);
            typename std::vector<typename T::iterator>::iterator inserted = main.insert(insert_pos, *pend_it);
            inserts_to_do--;
    ...
    for (ssize_t i = pend.size() - 1; i >= 0; i--) {
        ...
        typename std::vector<typename T::iterator>::iterator insert_pos = std::upper_bound(main.begin(), this_bound, *this_pend, compare<typename T::iterator>);
        main.insert(insert_pos, *this_pend);
    }
}
...
template <typename T> void PmergeMe::_merge_insertion_sort(T& container, int pair_size) {
	...
	CHANGE LAST AND END
	*/
    // last = one after last pair (boundary), end = first element of last pair
